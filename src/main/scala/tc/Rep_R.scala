...
import tc.RS_R_Main.NUM_PARTITIONS

import scala.collection.mutable.ListBuffer

/**
 * Spark TriangleCount using Rep (Map only) join with RDD.
 */
object Rep_R_Main {
  ...
  def main(args: Array[String]) {
    ...
    // Persist outgoing for rep join
    val outgoing = textFile
      .mapToPair(line => {
        val split = line.split(",")
        (split(0), split(1))
      }).filter(entry => entry._1.toInt < MAX && entry._2.toInt < MAX
    ).partitionBy(new HashPartitioner(NUM_PARTITIONS)).persist()
